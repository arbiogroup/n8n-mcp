...
export class SingleSessionHTTPServer {
  ...
  private sessionMetadata: {
    [sessionId: string]: { lastAccess: Date; createdAt: Date };
  } = {};
   ...
   */
  private sanitizeErrorForClient(error: unknown): {
    message: string;
    code: string;
  } {
  ...
  private validateEnvironment(): void {
    try {
      this.oauthConfig = loadOAuthConfig();
      this.jwtService = new JWTService(this.oauthConfig);
      this.oauthMiddleware = new OAuthMiddleware(this.jwtService);
      this.oauthHandlers = new OAuthHandlers(this.jwtService, this.oauthConfig);

      logger.info(
        "OAuth system initialized successfully for single session server"
      );
    } catch (error) {
      logger.error("Failed to initialize OAuth system", { error });
      throw new Error(
        "Failed to initialize OAuth system. Please check your OAuth environment variables."
      );
    }
  }
    ...
    return this.consoleManager.wrapOperation(async () => {
      try {
        ...
        if (isInitialize) {
          ...
          logger.info(
            "handleRequest: Creating new transport for initialize request"
          );
          ...
          transport = new StreamableHTTPServerTransport({
            ...
            onsessioninitialized: (initializedSessionId: string) => {
              ...
              logger.info(
                "handleRequest: Session initialized, storing transport and server",
                {
          ...
          transport.onclose = () => {
            ...
            if (sid) {
              logger.info("handleRequest: Transport closed, cleaning up", {
                sessionId: sid,
              });
          ...
          transport.onerror = (error: Error) => {
            ...
            if (sid) {
              this.removeSession(sid, "transport_error").catch((err) => {
                logger.error("Error during transport error cleanup", {
                  error: err,
                });
              });
            }
          ...
          if (!this.isValidSessionId(sessionId)) {
            logger.warn("handleRequest: Invalid session ID format", {
              sessionId,
            });
            res.status(400).json({
              jsonrpc: "2.0",
          ...
          logger.info("handleRequest: Reusing existing transport for session", {
            sessionId,
          });
          ...
          logger.warn(
            "handleRequest: Invalid request - no session ID and not initialize",
            errorDetails
          );
  ...
  private async resetSessionSSE(res: express.Response): Promise<void> {
    ...
    if (this.session) {
      try {
        logger.info("Closing previous session for SSE", {
    ...
    try {
      ...
      logger.info("Created new SSE session successfully", {
        sessionId: this.session.sessionId,
      });
    } catch (error) {
  ...
  async start(): Promise<void> {
    ...
    app.use(session(createSessionConfig(this.oauthConfig)));
    ...
    const googleStrategy = createGoogleStrategy(this.oauthConfig);
    ...
    app.get("/health", (req, res) => {
      ...
      res.json({
        ...
        security: {
          ...
          oauthEnabled: true,
          authenticationType: "OAuth 2.0 with JWT",
        },
    ...
    app.get(
      ...
      this.oauthHandlers.handleGoogleCallback
    );

    app.get("/auth/verify", this.oauthHandlers.verifyToken);
    ...
    app.get("/mcp", async (req, res) => {
      ...
      if (accept && accept.includes("text/event-stream")) {
        logger.info(
          "SSE stream request received - establishing SSE connection"
        );
    ...
    app.delete(
      ...
      async (req: express.Request, res: express.Response): Promise<void> => {
        ...
        if (this.transports[mcpSessionId]) {
          logger.info("Terminating session via DELETE request", {
    ...
    app.post(
      "/mcp",
      jsonParser,
      this.oauthMiddleware.authenticateToken,
      async (req: express.Request, res: express.Response): Promise<void> => {
        ...
        if (typeof req.on === "function") {
          const closeHandler = () => {
            if (!res.headersSent && sessionId) {
              logger.info("Connection closed before response sent", {
                sessionId,
              });
        ...
        logger.info(
          "OAuth authentication successful - proceeding to handleRequest",
          {
    ...
    this.expressServer = app.listen(port, host, () => {
      ...
      console.log(
        `Session Limits: ${MAX_SESSIONS} max sessions, ${
      ...
      if (isProduction) {
        console.log(
          "🔒 Running in PRODUCTION mode - enhanced security enabled"
        );
      } else {
